...
class Player:
	...
	# set position
	def set_position(self, mouse_pos, grid_cells, tile):
		for cell in grid_cells:
			if (cell.x * tile < mouse_pos[0] < (cell.x + 1) * tile and
                cell.y * tile < mouse_pos[1] < (cell.y + 1) * tile):
				#self.rect.center = (cell.x * tile + tile // 2, cell.y * tile + tile // 2)
				self.x, self.y = cell.x * tile + tile // 4, cell.y * tile + tile // 4
				self.update()
				break
